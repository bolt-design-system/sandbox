...
gulp.task('pattern-templates:copy', (done) => {
  globby(['./source/_framework/**/*.twig', '!node_modules']).then(patterns => {
...
gulp.task('pattern-docs:compile', (done) => {
  globby(['./source/_framework/**/README.md', '!node_modules']).then(patterns => {
    patterns.map((patternName) => {
      const dirName = path.dirname(patternName);
